...
DOCKER_MOUNT := $(if $(BIND_DIR),-v "$(CURDIR)/$(BIND_DIR):/go/src/github.com/alcideio/moby/$(BIND_DIR)")

# This allows the test suite to be able to run without worrying about the underlying fs used by the container running the daemon (e.g. aufs-on-aufs), so long as the host running the container is running a supported fs.
# The volume will be cleaned up when the container is removed due to `--rm`.
# Note that `BIND_DIR` will already be set to `bundles` if `DOCKER_HOST` is not set (see above BIND_DIR line), in such case this will do nothing since `DOCKER_MOUNT` will already be set.
DOCKER_MOUNT := $(if $(DOCKER_MOUNT),$(DOCKER_MOUNT),-v /go/src/github.com/alcideio/moby/bundles) -v $(CURDIR)/.git:/go/src/github.com/alcideio/moby/.git
...
manpages: ## Generate man pages from go source and markdown
	...
	docker run --rm \
		-v $(PWD):/go/src/github.com/alcideio/moby/ \
		docker-manpage-dev
...
.PHONY: swagger-gen
swagger-gen:
	docker run --rm -v $(PWD):/go/src/github.com/alcideio/moby \
		-w /go/src/github.com/alcideio/moby \
...
# For avoiding bakings DOCKER_GRAPHDRIVER and so on to image, we cannot use $(DOCKER_ENVS) here
	...
	docker exec $(tmp) go build -o /worker github.com/alcideio/moby/hack/integration-cli-on-swarm/agent/worker

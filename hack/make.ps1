...
Function Get-UpstreamCommit() {
    Invoke-Expression "git fetch -q https://github.com/alcideio/moby.git refs/heads/master"
...
Function Validate-DCO($headCommit, $upstreamCommit) {
    ...
    if ($badCommits.Length -eq 0) {
        ...
        $e += "https://github.com/alcideio/moby/blob/master/CONTRIBUTING.md#sign-your-work`n"
        Throw $e
    }
}
...
Function Validate-PkgImports($headCommit, $upstreamCommit) {
    ...
    $badFiles=@(); $files | %{
        ...
        $imports = $imports -NotMatch "^github.com/alcideio/moby/pkg/" `
                            -NotMatch "^github.com/alcideio/moby/vendor" `
                            -Match "^github.com/alcideio/moby" `
                            -Replace "`n", ""
...
Function Run-UnitTests() {
    ...
    $testPath="./..."
    $goListCommand = "go list -e -f '{{if ne .Name """ + '\"github.com/alcideio/moby\"' + """}}{{.ImportPath}}{{end}}' $testPath"
    $pkgList = $(Invoke-Expression $goListCommand)
    if ($LASTEXITCODE -ne 0) { Throw "go list for unit tests failed" }
    $pkgList = $pkgList | Select-String -Pattern "github.com/alcideio/moby"
    $pkgList = $pkgList | Select-String -NotMatch "github.com/alcideio/moby/vendor"
    $pkgList = $pkgList | Select-String -NotMatch "github.com/alcideio/moby/man"
    $pkgList = $pkgList | Select-String -NotMatch "github.com/alcideio/moby/integration-cli"
